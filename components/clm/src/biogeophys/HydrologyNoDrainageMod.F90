Module HydrologyNoDrainageMod
  ...
  public  :: HydrologyPostPFLOTRAN ! Snow and post soil water movement diagnostics after calling PFLOTRAN flow modules
...
contains
  ...
  !-----------------------------------------------------------------------
  subroutine HydrologyPostPFLOTRAN(bounds, &
       num_nolakec, filter_nolakec, &
       num_hydrologyc, filter_hydrologyc, &
       num_hydrononsoic, filter_hydrononsoic, &
       num_urbanc, filter_urbanc, &
    ...
    !
    ! !DESCRIPTION:
    ! This is the post-PFLOTRAN soil water movement
    ! Calling sequence is:
    ...
    use column_varcon        , only : icol_roof, icol_road_imperv, icol_road_perv, icol_sunwall
    use column_varcon        , only : icol_shadewall
    use clm_varctl           , only : use_cn, use_ed, use_pflotran, pf_hmode
    use clm_varpar           , only : nlevgrnd, nlevsno, nlevsoi, nlevurb
    use clm_time_manager     , only : get_step_size, get_nstep
    use SnowHydrologyMod     , only : SnowCompaction, CombineSnowLayers, DivideSnowLayers
    use SnowHydrologyMod     , only : SnowWater, BuildSnowFilter
    use SoilHydrologyMod     , only : CLMVICMap, WaterTable
    use SoilWaterRetentionCurveMod, only : soil_water_retention_curve_type
    !
    ! !ARGUMENTS:
    type(bounds_type)        , intent(in)    :: bounds
  ...
  end subroutine HydrologyPostPFLOTRAN
